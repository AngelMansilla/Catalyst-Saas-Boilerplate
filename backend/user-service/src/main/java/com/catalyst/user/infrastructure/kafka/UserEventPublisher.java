 ...
 * 
 * <p>
 * This component is only created if KafkaTemplate is available,
...
public class UserEventPublisher implements EventPublisher {
    ...
    public void publish(UserDeleted event) {
        log.info("Publishing UserDeleted event for user: {}", event.userId());

        Map<String, Object> payload = new HashMap<>();
        payload.put("userId", event.userId().getValue().toString());
        payload.put("email", event.email().getValue());

        publishEvent(userDeletedTopic, event.userId().toString(), event.eventId(),
                "UserDeleted", payload);
    }
